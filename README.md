...
腾讯 Marvis（马维斯）是操作系统级 AI 助手（文件 / 系统 / 应用 / 浏览器 / 搜索等内置 Agent）。截至目前，它**没有公开的自定义 skill / 知识库 / 插件（MCP）入口**，因此**无法像 WorkBuddy 那样正式安装本 skill**。

临时变通（非持久、不保证）：把本 skill 文件夹放在本地，让 Marvis 的**文件 Agent** 读取 `references/` 资料、并在对话中把 `SKILL.md` 的规则作为要求；若其版本允许执行命令，可让它运行 `python "<路径>/scripts/score_fund.py" <基金>` 等脚本。要正式支持，需等官方开放自定义 Agent / MCP / 知识库接口。建议优先用 **WorkBuddy** 或 **Claude Code**。
